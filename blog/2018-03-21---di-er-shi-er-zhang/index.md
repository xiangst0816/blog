...
* 即使执行完一个异步代码(Tasks）, **也会有一个很短的时间间隔**, 用于处理页面上的其他事情, 防止页面锁定
...
当在 Tasks 队列中**没有**该定时器的任何其他代码实例时，**才将定时器代码添加到队列中**。这确保定**时器代码被加入到 Tasks 队列中**的最短时间间隔为指定的间隔。<mark>这里是加入到 Tasks 队列，不是立即执行。</mark>
